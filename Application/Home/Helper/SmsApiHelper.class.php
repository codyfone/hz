...
//附加类
class SmsApiHelper extends Controller {
  ...
  public function __construct() {
    ...
    $this->userid = C('SMS_UID');
    $this->productid = C('SMS_PID');
    $this->sms_key = C('SMS_PASSWD');
  }
  ...
  public function send_sms($mobile = '', $content = '', $send_time = '', $charset = 'gbk', $id_code = '', $tplid = '', $return_code = 0) {
    //短信发送状态
    $status = $this->sms_status(0, 1);
    if (is_array($mobile)) {
      $mobile = implode(",", $mobile);
    }
    $content = $this->_safe_replace($content);
    ...
    $send_userid = $user ? $user['uid'] : 0;
    $ip = get_client_ip();

    $new_content = $content;
    if (isset($this->statuscode)) {
      M('Sms_report')->save(array('mobile' => $mobile, 'posttime' => NOW_TIME, 'id_code' => $id_code, 'send_userid' => $send_userid, 'status' => $this->statuscode, 'msg' => $new_content, 'return_id' => $return, 'ip' => $ip));
    } else {
      M('Sms_report')->save(array('mobile' => $mobile, 'posttime' => NOW_TIME, 'send_userid' => $send_userid, 'status' => '-2', 'msg' => $new_content, 'ip' => $ip));
    }
