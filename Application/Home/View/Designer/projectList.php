        ...
        <table class="m_table" style="width:100%">
          ...
          <tbody>
            <?php foreach ($new_info['rows'] as $v) { ?>
              <tr><td class="red" align="center">{$v['t1_code']}</td><td><?php if ($v['t1_exhibitin_id'] != 0) { ?><a href="{:U('Designer/index',['id'=>$v['t1_mid']])}" tareget="_blank">{$v['t1_company']}</a><?php } else { ?>{$v['t1_company']}<?php } ?></td><td><?php if ($v['t1_exhibitin_id'] != 0) { ?><a href="{:U('Zhanhui/view',['id'=>$v['t1_exhibitin_id']])}" tareget="_blank">{$v['t1_exhibition']}</a><?php } else { ?>{$v['t1_exhibition']}<?php } ?></td><td align="center">{$v['t1_floor_area']}m2</td><td>{$v['t2_enddate']}</td><td><a href="{:U('Designer/project',['id'=>$v['id']])}">详情</a></td></tr>
            <?php } ?>
          </tbody>
        </table>
