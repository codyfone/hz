          ...
          <table class="m_table" style="width:100%">
            ...
            <tbody>
              <?php foreach ($new_info['rows'] as $v) { ?>
                <tr><td class="red" align="center"><?= $v['t1_code'] ?></td><td><a href="<?= U('Sheji/anli',['id'=>$v['id']])?>"><?= $v['title'] ?><span class="red">[预览]</span></a></td><td align="center">{$v['floor_area']}m2</td><td align="center" width="145">{$v['addtime']}</td><td align="center"><a href="{:U('Factory/product',['act'=>'edit','id'=>$v['id']])}">修改</a></td></tr>
              <?php } ?>
            </tbody>
          </table>
        <?php } else { ?>
