...
//检查用户登录及权限并返回值
    ...
    if ($info === 0) {
      ...
      $this->success('此页面需要登录后才能访问！', U(MODULE_NAME . '/Index/login'));exit;  //主页登陆失败，THINKPHP返回登陆页
    } elseif ($info == -99) {
      echo "<script>$.messager.alert('提示','您尚未登录！','warning',function(){window.location='" . U(MODULE_NAME . '/Index/login') . "'});</script>";
      exit;                  //主页登陆失败，EASYUI返回登陆页
    } elseif ($info == -100) {
      $this->success('您的账号被拒绝登录！', U(MODULE_NAME . '/Index/login'));  //主页登陆失败，THINKPHP返回登陆页																	//已关闭的用户登录时返回
