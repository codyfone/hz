...
//      $info = $Project->field('GROUP_CONCAT(id) as id')->where('`status`=65 and TO_DAYS(NOW()) - TO_DAYS(`uptime`)>' . $expire)->find();
//
...
//      }
  ...
  public function getLog($pid, $id, $type) {
    ...
    //main
    $project = M('Project');
    $pinfo = $project->field('status,uptime')->where('id=' . $pid)->find();
