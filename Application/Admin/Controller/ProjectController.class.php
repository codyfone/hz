...
//          dump($info);
      ...
      $data['baseinfo']['content'] = I('content', '', false);
...
//          $Sms->sendsms($sms_data, $data['pm_id']);
        ...
        if (!is_int((int) $id)) {
          ...
          if ($edit !== false) {
            if ($edit == 1) {
              ...
              $uid = $Design->field('mid as user_id')->where('`pro_id`=' . $id)->select();
  ...
  public function del() {
    ...
    foreach ($arr_id as $id) {
      ...
      $uid = $design->field('mid')->where('`pro_id`=' . $id)->select();
  ...
  public function detail($id, $tid = NULL) {
    ...
    if (!is_int((int) $id)) {
      ...
      $map['id'] = array('eq', $id);
      $info = $project->field('company,exhibition')->where($map)->find();
      //   print_r($info);exit;
        ...
        case 'add':
          ...
          if ($add > 0) {
            if ($des_id) {
              ...
              $Design = M('Design');
              $Linkage = M('Linkage');
              $pro = $Project->filed('company,exhibition')->where('id=' . $pro_id)->find();
              ...
              $sms_data = array(
                ...
                'description' => '<p>' . $data['description'] . '</p> <p>相关方案：“<a href="javascript:showTab(\'项目-' . $proname . '\',' . $pro_id . ',' . $des_id . ');">' . $designname . '</a>”，点击方案名称查看更多详情。</p>',
              );

              $mid = $Design->where('`id`=' . $des_id)->getField('mid');
              ...
              $uid = $Design->field('mid')->where('`pro_id`=' . $pro_id)->select();
              $Sms->sendsms($sms_data, $uid);
            }
            echo 1;
          } else {
  ...
  public function designlist($pid = 0, $json = NULL, $method = NULL) {
    ...
    if ($json == 1) {
      ...
      if (!$view) {
        $info = $result->table($Design_table . ' as t1')->field('SQL_CALC_FOUND_ROWS ' . $fields)->join('LEFT JOIN ' . $user_table . ' as t4 on t4.id = t1.mid')->join('LEFT JOIN ' . $linkage . ' as t3 on t3.id = t1.status')->having($map)->order($sort . ' ' . $order)->limit($offset, $rows)->select();
        $count = $result->query('SELECT FOUND_ROWS() as total');
        $count = $count[0]['total'];
      } else {
        $info = $result->table($Design_table . ' as t1')->field($fields)->join('LEFT JOIN ' . $user_table . ' as t4 on t4.id = t1.mid')->join('LEFT JOIN ' . $linkage . ' as t3 on t3.id = t1.status')->having($map)->order($sort . ' ' . $order)->select();
        $count = count($info);
      }
      //dump($info);exit;
      $new_info = array();
    ...
    if ($go == 1) {
      if ($act == 'add') {
        ...
        if ($add > 0) {
          ...
          $mid = $Design->where('id=' . $data['main']['des_id'])->getField('mid');
          $sms_data = array(
            'title' => '项目：' . $proname . ' 有新评论',
            'description' => '<p>' . $data['description'] . '</p> <p>相关方案：“<a href="javascript:showTab(\'项目-' . $proname . '\',' . $data['main']['pro_id'] . ',' . $data['main']['des_id'] . ');">' . $designname . ' ' . $data['addtime'] . '工作日志</a>”，点击方案名称查看更多详情。</p>',
          );
          $Sms->sendsms($sms_data, $mid);
          echo 1;
        } else {
