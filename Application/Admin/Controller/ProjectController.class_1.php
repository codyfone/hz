...
//    $this->assign('protype', $protype);
  ...
  public function projectlist($type, $json = NULL, $method = NULL) {
    ...
    if ($json == 1) {
      ...
      $sort = $get_sort ? strval($get_sort) : 't1_old_pass';
      ...
      $arr_flelds = array(
        ...
        'company' => 't1.company as t1_old_company',
      ...
      if (!$view) {
        if ($all) {
          $info = $result->table($Project_table . ' as t1')->field($fields)->join('LEFT JOIN ' . $Project_baseinfo . ' as t2 on t2.pro_id = t1.id')->join('LEFT JOIN ' . $linkage . ' as t4 on t4.id = t1.status')->having($map)->order($sort . ' ' . $order . ',t1_old_uptime desc')->limit($offset, $rows)->select();
          $count = $result->query('select count(*) as total from ' . $Project_table);
        } else {

          $info = $result->table($Project_table . ' as t1')->field('SQL_CALC_FOUND_ROWS ' . $fields)->join('LEFT JOIN ' . $Project_baseinfo . ' as t2 on t2.pro_id = t1.id')->join('LEFT JOIN ' . $linkage . ' as t4 on t4.id = t1.status')->having($map)->order($sort . ' ' . $order . ',t1_old_uptime desc')->limit($offset, $rows)->select();
          $count = $result->query('SELECT FOUND_ROWS() as total');
        }
        $count = $count[0]['total'];
      } else {
        $info = $result->table($Project_table . ' as t1')->field($fields)->join('LEFT JOIN ' . $Project_baseinfo . ' as t2 on t2.pro_id = t1.id')->join('LEFT JOIN ' . $linkage . ' as t4 on t4.id = t1.status')->having($map)->order($sort . ' ' . $order . ',t1_old_uptime desc')->select();
        $count = count($info);
      }
      //dump($info);exit;
      $new_info = array();
        ...
        if (!is_int((int) $id)) {
          ...
          if ($edit !== false) {
            if ($edit == 1) {
              ...
              $fid = '(' . $Design->field('from_id as user_id')->where('`pro_id`=' . $id . ' and `from_id`<>' . $data['pm_id'])->group('from_id')->order('from_id')->select(false) . ')';
              $uid = $Design->field('to_id as user_id')->where('`pro_id`=' . $id . ' and `to_id`<>' . $data['pm_id'])->union($fid)->select();
  ...
  public function del() {
    ...
    $project = D('Project');
    $design = M('Design');
    $designmain = M('Design_main');
    $designbase = M('Design_baseinfo');
    ...
    foreach ($arr_id as $id) {
      ...
      $fid = '(' . $design->field('from_id as user_id')->where('`pro_id`=' . $id . ' and `from_id`<>' . $pm_id)->group('from_id')->order('from_id')->select(false) . ')';
      $uid = $design->field('to_id as user_id')->where('`pro_id`=' . $id . ' and `to_id`<>' . $pm_id)->union($fid)->select();
      ...
      if ($del) {
        $map = array();
        $Log->moveLog($id, 1);
        $sql = '(' . $designmain->field('des_id as id')->where('`pro_id`=' . $id)->select(false) . ')';
        $map['id'] = array('exp', ' IN(' . $sql . ')');
        $maps['des_id'] = array('exp', ' IN(' . $sql . ')');
        $design->where($map)->delete();
        $designbase->where($maps)->delete();
        $designmain->where('`pro_id`=' . $id)->delete();
  ...
  public function content($id, $tid = NULL) {
    ...
    cookie('aFiles', NULL);
    $project = D('Project');
    $design = D('Design_table');
    $design_main = D('Design_main_table');
    ...
    if (!is_int((int) $id)) {
      ...
      if ($tid == NULL) {
        ...
        $ids = $design_main->where('pro_id=' . $id)->getField('GROUP_CONCAT(des_id ORDER BY pro_id) as id');
        $count = $design_main->where('pro_id=' . $id)->getField('count(id) as total');
        if ($ids) {
          $comple = $design->where('id IN(' . $ids . ') and status=51')->getField('count(id) as comple');
          $ing = $design->where('id IN(' . $ids . ') and status>9')->getField('count(id) as ing');
        } else {
          $comple = $ing = 0;
        }

        $maxdate = $design->where('`pro_id`=' . $id)->getField('MAX(enddate)');
      ...
      switch ($act) {
        case 'add':
          ...
          if ($add > 0) {
            if ($des_id) {
              $Project = M('Project');
              $Design = M('Design_table');
              $Linkage = M('Linkage');
              $proname = $Project->where('id=' . $pro_id)->getField('title');
              $designname = $Design->where('id=' . $des_id)->getField('title');
              $pm_id = $Project->where('id=' . $pro_id)->getField('pm_id');
              $sms_data = array(
                'title' => '项目：' . $proname . ' 有新评论',
                'description' => '<p>' . $data['description'] . '</p> <p>相关任务：“<a href="javascript:showTab(\'项目-' . $proname . '\',' . $pro_id . ',' . $des_id . ');">' . $designname . '</a>”，点击任务名称查看更多详情。</p>',
              );

              $from_id = $Design->where('`id`=' . $des_id . ' and `from_id`<>' . $pm_id)->getField('from_id');
              $to_id = $Design->where('`id`=' . $des_id . ' and `to_id`<>' . $pm_id)->getField('to_id');
              $uid = array(
                0 => array(
                  'user_id' => $to_id
                ),
              ...
              $fid = '(' . $Design->field('from_id as user_id')->where('`pro_id`=' . $pro_id . ' and `from_id`<>' . $pm_id)->group('from_id')->order('from_id')->select(false) . ')';
              $uid = $Design->field('to_id as user_id')->where('`pro_id`=' . $pro_id . ' and `to_id`<>' . $pm_id)->union($fid)->select();
              array_unshift($uid, array(
                'user_id' => $pm_id,
              ));
   ...
   * @examlpe 
   */
  public function design($act, $go = 0, $id = 0, $tid = 0) {
    $Public = A('Index', 'Helper');
    ...
    if ($go == 1) {
      ...
      switch ($act) {
        case 'add':
          $role = $Public->check('Design', array('c'));
          if ($role < 0) {
            echo $role;
            exit;
          }
        ...
        case 'edit':
          $role = $Public->check('Design', array('u'));
          if ($role < 0) {
            echo $role;
            exit;
          }
          ...
          if ($edit !== false) {
            ...
            if ($edit == 1) {
              ...
              $sms_data = array(
                ...
                'description' => $username . '更新了任务：“<a href="javascript:showTab(\'项目-' . $proname . '\',' . $pro_id . ',' . $des_id . ');">' . $data['title'] . '</a>”，点击任务名称查看更多详情。',
              );
              if ($data['to_id'] != $pm_id) {
                $uid[] = $data['to_id'];
              }
            ...
            if ($data['to_id'] != $oldto) {
              $sms_data = array(
                'title' => '项目：' . $proname . ' 任务指派通知',
                'description' => $username . '指派了您为任务：“<a href="javascript:showTab(\'项目-' . $proname . '\',' . $pro_id . ',' . $des_id . ');">' . $data['title'] . '</a>” 的任务负责人，点击任务名称查看更多详情。',
              );
              $Sms->sendsms($sms_data, $data['to_id']);
            }
        ...
        case 'del':
          $role = $Public->check('Design', array('d'));
          if ($role < 0) {
            echo $role;
            exit;
          }
          ...
          $to_id = $design->where($map)->getField('to_id');
          ...
          if ($del == 1) {
            ...
            $sms_data = array(
              ...
              'description' => $username . '刪除了任务：“' . $designbame . '”。',
            );
            if ($to_id != $pm_id) {
              $uid[] = $to_id;
            }
   ...
   * @examlpe 
   */
  public function getDesign($pid) {
    $Left = A('Design', 'Helper');

    //main
    if (is_int((int) $pid)) {
      $info = $Left->rowDesign($pid);
  ...
  public function check($type) {
    $Public = A('Index', 'Helper');
    $role = $Public->check('Design', array('p'));
    $Log = A('Log', 'Helper');
    if ($role < 0) {
      echo $role;
      exit;
    ...
    $edit = $design->where($map)->save($data);
    if ($edit !== false) {
      $log_data = array(
        'pro_id' => $pro_id,
        'des_id' => $id,
   ...
   * @examlpe 
   */
  public function designlist($pid = 0, $json = NULL, $method = NULL) {
    $Public = A('Index', 'Helper');
    ...
    if ($json == 1) {
      ...
      $result = M();
      $Design_main = M('design_main_table');
      $Design_table = C('DB_PREFIX') . 'design_table';
      $Worklog_table = C('DB_PREFIX') . 'worklog_table';
      ...
      $map = array();
      if ($pid) {
        $sql = '(' . $Design_main->field('des_id as id')->where('pro_id=' . $pid)->select(false) . ')';
        $map['id'] = 'id in(' . $sql . ')';
      } else {
        $map['id'] = 'id>0';
      }
      ...
      $arr_flelds = array(
        'id' => 't1.id as id',
        'title' => 't1.company as t1_old_company',
      ...
      if (!$view) {
        $info = $result->table($Design_table . ' as t1')->field('SQL_CALC_FOUND_ROWS ' . $fields)->join('LEFT JOIN ' . $user_table . ' as t2 on t2.id = t1.to_id')->join('LEFT JOIN ' . $user_table . ' as t4 on t4.id = t1.from_id')->join('LEFT JOIN ' . $linkage . ' as t6 on t6.id = t1.level')->join('LEFT JOIN ' . $linkage . ' as t5 on t5.id = t1.degree')->join('LEFT JOIN ' . $linkage . ' as t3 on t3.id = t1.status')->having($map)->order($sort . ' ' . $order)->limit($offset, $rows)->select();
        $count = $result->query('SELECT FOUND_ROWS() as total');
        $count = $count[0]['total'];
      } else {
        $info = $result->table($Design_table . ' as t1')->field($fields)->join('LEFT JOIN ' . $user_table . ' as t2 on t2.id = t1.to_id')->join('LEFT JOIN ' . $user_table . ' as t4 on t4.id = t1.from_id')->join('LEFT JOIN ' . $linkage . ' as t6 on t6.id = t1.level')->join('LEFT JOIN ' . $linkage . ' as t5 on t5.id = t1.degree')->join('LEFT JOIN ' . $linkage . ' as t3 on t3.id = t1.status')->having($map)->order($sort . ' ' . $order)->select();
        $count = count($info);
      }
      //dump($info);exit;
      $new_info = array();
      ...
      if ($method == 'total') {
        ...
        header("Expires: 0");
        //导出xls 开始
        $title = array('任务名称', '任务负责人', '来自', '任务状态', '计划开始日', '计划完成日', '任务进度', '计划用时', '已用工时', '更新时间');
    ...
    if ($go == 1) {
      ...
      switch ($act) {
        case 'add':
          ...
          $check = $design->where('id=' . $des_id)->getField('check');
          if ($check) {
            echo 2;
            exit;
          }
          ...
          if ($add > 0) {
            ...
            $edit = $design->where('id=' . $des_id)->save($tdata);
            $linkage = M('Linkage');
            $statusname = $linkage->where('id=' . $data['status'])->getField('text');
    ...
    if ($go == 1) {
      if ($act == 'add') {
        ...
        if ($add > 0) {
          $Project = M('Project');
          $Design = M('Design_table');
          $proname = $Project->where('id=' . $data['main']['pro_id'])->getField('company');
          $designname = $Design->where('id=' . $data['main']['des_id'])->getField('title');
          $pm_id = $Project->where('id=' . $data['main']['pro_id'])->getField('pm_id');
          $to_id = $Design->where('id=' . $data['main']['des_id'])->getField('to_id');
          $from_id = $Design->where('id=' . $data['main']['des_id'] . ' and `from_id`<>' . $to_id)->getField('from_id');
          $sms_data = array(
            'title' => '项目：' . $proname . ' 有新评论',
            'description' => '<p>' . $data['description'] . '</p> <p>相关任务：“<a href="javascript:showTab(\'项目-' . $proname . '\',' . $data['main']['pro_id'] . ',' . $data['main']['des_id'] . ');">' . $designname . ' ' . $data['addtime'] . '工作日志</a>”，点击任务名称查看更多详情。</p>',
          );
  ...
  public function worklogpie($id) {
    ...
    $info = $result->table($Design_table . ' as t1')->field('t2.text as name,concat_ws(\'\',' . $sql . ') as worktime')->join('LEFT JOIN ' . $linkage . ' as t2 on t2.id=t1.type')->where('t1.pro_id=' . $id)->order('t1.type')->group('t1.type')->select();
