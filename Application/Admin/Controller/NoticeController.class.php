...
class NoticeController extends Controller {
  ...
  public function index($json = NULL, $method = NULL) {
    ...
    if ($json == 1) {
      ...
      $sort = $get_sort ? strval($get_sort) : 't1_old_addtime';
      ...
      if (!$view) {
        if ($all) {
          $info = $result->table($Notice_table . ' as t1')->field($fields)->join('LEFT JOIN ' . $user_table . ' as t2 on t2.id = t1.user_id')->having($map)->order($sort . ' ' . $order)->limit($offset, $rows)->select();
          $count = $result->query('select count(*) as total from ' . $Notice_table);
        } else {
          $info = $result->table($Notice_table . ' as t1')->field('SQL_CALC_FOUND_ROWS ' . $fields)->join('LEFT JOIN ' . $user_table . ' as t2 on t2.id = t1.user_id')->having($map)->order($sort . ' ' . $order)->limit($offset, $rows)->select();
          $count = $result->query('SELECT FOUND_ROWS() as total');
        }
        $count = $count[0]['total'];
      } else {
        $info = $result->table($Notice_table . ' as t1')->field($fields)->join('LEFT JOIN ' . $user_table . ' as t2 on t2.id = t1.user_id')->having($map)->order($sort . ' ' . $order)->select();
        $count = count($info);
      }
      //dump($info);exit;
      $new_info = array();
