...
class UserController extends Controller {
  ...
  public function index($json = NULL, $method = NULL) {
    ...
    if ($json == 1) {
      ...
      $sort = $get_sort ? strval($get_sort) : 'username'; //，默认排序字段
      ...
      if (!$view) {//不开启视图
        $info = $result->table($user_table . ' as t1')->field('SQL_CALC_FOUND_ROWS ' . $fields)->join('LEFT JOIN ' . $user_main . ' as t5 on t5.user_id = t1.id')->join('LEFT JOIN ' . $group_table . ' as t2 on t2.id = t5.group_id')->join('LEFT JOIN ' . $part_table . ' as t3 on t3.id = t5.part_id')->join('LEFT JOIN ' . $comy_table . ' as t4 on t4.id = t5.company_id')->having($map)->order($new_order)->limit($offset, $rows)->select();
        $count = $result->query('SELECT FOUND_ROWS() as total');
        $count = $count[0]['total'];
      } else {//开启视图
        $info = $result->table($user_table . ' as t1')->field($fields)->join('LEFT JOIN ' . $user_main . ' as t5 on t5.user_id = t1.id')->join('LEFT JOIN ' . $group_table . ' as t2 on t2.id = t5.group_id')->join('LEFT JOIN ' . $part_table . ' as t3 on t3.id = t5.part_id')->join('LEFT JOIN ' . $comy_table . ' as t4 on t4.id = t5.company_id')->having($map)->order($new_order)->select();
        $count = count($info);
      }
      //dump($info);exit;
      $new_info = array();
