...
class LogController extends Controller {
  ...
  public function index($json = NULL, $pid = NULL, $method = NULL) {
    ...
    if ($json == 1) {
      ...
      $Porject_table = C('DB_PREFIX') . 'project';
      ...
      if (!$view) {
        $info = $result->table($Log_table . ' as t1')->field('SQL_CALC_FOUND_ROWS ' . $fields)->join('LEFT JOIN ' . $Log_main . ' as t2 on t2.log_id = t1.id')->join('LEFT JOIN ' . $User_table . ' as t3 on t3.id = t1.user_id')->join(' right join ' . $Task_table . ' as t5 on t5.id = t2.task_id')->join('LEFT JOIN ' . $Linkage . ' as t4 on t4.id = t5.type')->join('LEFT JOIN ' . $Linkage . ' as t6 on t6.id = t1.status')->join(' right join ' . $Porject_table . ' as t7 on t7.id = t2.pro_id')->having($map)->order('addtime desc')->limit($offset, $rows)->select();
        $count = $result->query('SELECT FOUND_ROWS() as total');
        $count = $count[0]['total'];
      } else {
        $info = $result->table($Log_table . ' as t1')->field($fields)->join('LEFT JOIN ' . $Log_main . ' as t2 on t2.log_id = t1.id')->join('LEFT JOIN ' . $User_table . ' as t3 on t3.id = t1.user_id')->join('LEFT JOIN ' . $Task_table . ' as t5 on t5.id = t2.task_id')->join('LEFT JOIN ' . $Linkage . ' as t4 on t4.id = t5.type')->join('LEFT JOIN ' . $Linkage . ' as t6 on t6.id = t1.status')->join('LEFT JOIN ' . $Porject_table . ' as t7 on t7.id = t2.pro_id')->having($map)->order('addtime desc')->select();
        $count = count($info);
      }
      //dump($info);exit;
      $new_info = array();
