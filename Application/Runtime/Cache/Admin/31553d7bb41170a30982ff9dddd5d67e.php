...
$(function(){
	...
	$("#view<?php echo ($uniqid); ?>").combobox({
		required:false,
		url:'/Application/Runtime/Data/Json/User_noadmin_data.json',
		editable:true,
		multiple:true,
		method:'get',
		valueField:'id',
...
function onSubmitMenu(idd){
	...
	$("#addFormMenu"+idd).form('submit',{
		url:'/index.php/Admin/Menu/add/act/add/go/1',
		onSubmit: function(){
...
function onUploadMenu(idd){
	...
	var ids = $("#ids"+idd).val();
	$("#addFormMenu"+idd).form('submit',{
		url:'/index.php/Admin/Menu/add/act/edit/go/1/id/'+ids,
		onSubmit: function(){
			var isValid = $(this).form('validate');
			if (!isValid){
				$.messager.progress('close');
...
<div class="con-tb">
<form id="addFormMenu<?php echo ($uniqid); ?>" method="post">
 <table class="infobox" width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td width="23%"><label for="_parentId">上级菜单</label></td>
    <td width="77%">
    <input name="_parentId" class="easyui-combotree relo" id="parentid" size="28" data-options="url:'/Application/Runtime/Data/Json/Menu_data.json',editable:true,lines:true,method:'get'" /><input id="ids<?php echo ($uniqid); ?>" type="hidden" value="<?php echo ($id); ?>" />
    </td>
  </tr>
  <tr>
    <td width="23%"><label for="text">菜单名称</label></td>
    <td width="77%"><input name="text" type="text" class="easyui-validatebox" size="28" data-options="required:true" /></td>
  </tr>
  <tr>
    <td width="23%"><label for="code">识别码</label></td>
    <td width="77%"><input name="code" type="text" class="easyui-validatebox" size="22" data-options="required:false" /> (对应控制器名称)</td>
  </tr>
  <tr>
    <td><label for="url">链接</label></td>
    <td><input name="url" type="text" class="easyui-validatebox" size="38" /></td>
  </tr>
  <tr>
    <td width="23%"><label for="iconCls">标题图片</label></td>
    <td width="77%"><input name="iconCls" type="text" class="easyui-validatebox" size="22" /></td>
  </tr>
  <tr>
    <td width="23%"><label for="state">展开状态</label></td>
    <td width="77%">
  ...
  <tr>
    <td width="23%"><label for="level">查看权限</label></td>
    <td width="77%">
    <select class="easyui-combobox" name="mode">  
        <option value="1">组别</option>  
        <?php if(C('MORE_COMY')): ?><option  value="2">公司</option><?php endif; ?>
    ...
    <input name="level" id="level<?php echo ($uniqid); ?>" type="text" class="easyui-validatebox" size="16" data-options="" /></td>
  </tr>
  <tr>
    <td width="23%"><label for="view">开启用户</label></td>
    <td width="77%"><input name="view[]" id="view<?php echo ($uniqid); ?>" type="text" size="38" /></td>
  </tr>
  <tr>
