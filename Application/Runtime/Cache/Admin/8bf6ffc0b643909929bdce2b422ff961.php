...
$(function(){
	...
	$("#Notice").datagrid({
		...
		sortOrder:'desc',
		url:'/index.php/Admin/Notice/index/json/1',
		...
		onBeforeLoad: function () {  
			 if($("#NoticeCon .datagrid-toolbar table tr #sersSearchNotice").length==0){
				 ...
				 $("#sersSearchNotice").combotree({
					...
					onClick:function(q){
						$.post('/index.php/Admin/Notice/change/act/user_id', {val:q.id}, function(data){
		...
		onHeaderContextMenu:function(e,f){
			if(f!='contents'){
				$("#searchNotice").dialog({
					...
					width:430,
					height:80,
					href:'/index.php/Admin/Notice/search/field/'+f
				});
			}
		...
		onDblClickRow:function(e,rowIndex,rowData){
			...
			if(se_len==1 && !has){
				$("<div/>").dialog({
					...
					width:720,
					height:480,
					href:'/index.php/Admin/Notice/detail/id/'+idd,
					onOpen:function(){
		...
		iconCls: 'icon-add',
			...
			handler: function(){
				$("#addNotice").dialog({
					...
					width:720,
					height:480,
					href:'/index.php/Admin/Notice/add/act/add',
					onOpen:function(){
						cancel['Notice'] = $(this);
					},
					onClose:function(){
			...
			handler: function(){
				...
				if(se_len==1){
					$("#addNotice").dialog({
						...
						width:720,
						height:480,
						href:'/index.php/Admin/Notice/add/act/edit/id/'+idd,
						onOpen:function(){
							cancel['Notice'] = $(this);
						},
						onClose:function(){
			...
			handler: function(){
				...
				if(s){
					$.messager.confirm('提示','确定删除吗！',function(r){
						if(r==true){
							$.messager.progress();
							$.post('/index.php/Admin/Notice/del', {id:s}, function(data){
			...
			handler: function(){
				$("#searchNotice").dialog({
					...
					width:500,
					height:220,
					href:'/index.php/Admin/Notice/advsearch'
				});
			}
		},'-',{
			iconCls: 'icon-reload',
			text : '重载',
			handler: function(){
				$.get('/index.php/Admin/Notice/clear', function(data){
...
<div class="con" id="NoticeCon" onselectstart="return false;" style="-moz-user-select:none;">
 <?php if(C('DATAGRID_VIEW')!='0'): ?><table id="Notice" data-options="view:<?php echo C("DATAGRID_VIEW") ?>"></table>
 <?php else: ?>
 <table id="Notice"></table><?php endif; ?>
</div>
<div id="searchNotice"></div>
<div id="addNotice"></div>
<div id="selectInputNotice" style="display:none">
 <span class="datagrid-btn-separator-nofloat"  style="margin-right:2px;"></span>
 <input id="sersSearchNotice" style="width:128px; margin-left:3px" />
</div>