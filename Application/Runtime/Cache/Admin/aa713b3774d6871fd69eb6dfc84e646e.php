...
$(function(){
	...
	$("#Group").datagrid({
		...
		rownumbers:true,
		method:'get',
		url:'/index.php/Admin/MemberGroup/index/json/1',
		fitColumns:true,
		...
		onDblClickRow:function(e,rowIndex,rowData){
			...
			if(idd>1 && idd!=gidd){
				$("#addGroup").dialog({
					...
					width:450,
					height:210,
					href:'/index.php/Admin/MemberGroup/add/act/edit/id/'+idd,
					onOpen:function(){
						cancel['Group'] = $(this);
					},
					onClose:function(){
		...
		iconCls: 'icon-add',
			...
			handler: function(){
				$("#addGroup").dialog({
					...
					width:450,
					height:210,
						href:'/index.php/Admin/MemberGroup/add/act/add',
					onOpen:function(){
						cancel['Group'] = $(this);
					},
					onClose:function(){
			...
			handler: function(){
				...
				if(idd>1){
					$.messager.confirm('提示','確定刪除吗！',function(r){
						if(r==true){
							$.messager.progress();
							$.get('/index.php/Admin/MemberGroup/del/id/'+idd, function(data){
			...
			text : '更新緩存',
			handler: function(){
				$.get('/index.php/Admin/MemberGroup/json', function(data){
					if(data==1){
		...
		columns:[[   
			{field:'name',title:'组别',width:250}, 
			{field:'modelid',title:'类型',width:120},   
			{field:'status',title:'状态',width:90},
			{field:'description',title:'备注',width:650}
		]]
	});
});
</script>
