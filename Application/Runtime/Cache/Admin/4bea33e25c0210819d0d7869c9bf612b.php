...
function onSubmitUser(idd){
	...
	$("#addFormUser"+idd).form('submit',{
		url:'/index.php/Admin/Designer/add/act/add/go/1',
		onSubmit: function(){
...
function onUploadUser(idd){
	...
	var ids = $("#ids"+idd).val();
	$("#addFormUser"+idd).form('submit',{
		url:'/index.php/Admin/Designer/add/act/edit/go/1/id/'+ids,
		onSubmit: function(){
			var isValid = $(this).form('validate');
			if (!isValid){
				$.messager.progress('close');
...
function onRepwd(idd){
	...
	var ids = $("#ids"+idd).val();
	$("#addFormUser"+idd).form('submit',{
		url:'/index.php/Admin/Designer/rspwd/id/'+ids,
		onSubmit: function(){
			var isValid = $(this).form('validate');
			if (!isValid){
				$.messager.progress('close');
...
<form id="addFormUser<?php echo ($uniqid); ?>" method="post">
 <table class="infobox" width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td width="23%"><label for="nickname">姓名</label></td>
    <td width="77%"> <input name="nickname" type="text" class="easyui-validatebox" size="10" data-options="required:true" /></td>
  </tr>
  <tr>
