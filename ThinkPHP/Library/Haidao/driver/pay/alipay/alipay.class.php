...
namespace Haidao\driver\pay\alipay;

use Haidao\pay_abstract;
...
class alipay extends pay_abstract {
  ...
  public function __construct($config = array()) {
    ...
    $this->config['notify_url'] = return_url('alipay', 'notify');
    $this->config['return_url'] = return_url('alipay', 'return');
  }
  ...
  public function getpreparedata() {
    ...
    if (array_key_exists('url', $this->product_info))
      $prepare_data['show_url'] = $this->product_info['url'];
  ...
  private function filterParameter($parameter) {
    ...
    foreach ($parameter as $key => $value) {
      if ('sign' == $key || 'sign_type' == $key || '' == $value || 'm' == $key || 'a' == $key || 'c' == $key || 'code' == $key || 'method' == $key || 'page' == $key)
        continue;
      else
        $para[$key] = $value;
    }
